...
<html>
    ...
    <body>
        ...
        <div class="row">
          <div class="col-md-4">
                <div class="thumbnail">
                  <a href="${appUtil.patchUrl(image.dataUrl, request)}">
                    <img src="${appUtil.patchUrl(image.imageUrl, request)}" alt="LGTM"/>
                  </a>
                </div>
          </div>
          <div class="col-md-8">
            <form>
                ...
                <input type="text" value="${appUtil.patchUrl(image.imageUrl, request)}"
                       class="form-control" id="imageUrl" name="imageUrl">
                </div>
                <div class="form-group>
                <label for="dataUrl">Data Url</label>
                <input type="text" value="${appUtil.patchUrl(image.dataUrl, request)}"
                       class="form-control" id="dataUrl" name="dataUrl">
                </div>
                <div class="form-group>
                <label for="markdown">Markdown</label>
                <textarea class="form-control" rows="4" id="markdown" name="markdown"
                        >${appUtil.patchUrl(image.markdown.replace("\\n", '\n'), request)}</textarea>
                </div>
